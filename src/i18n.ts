...
const SETTINGS_I18N: Partial<Record<SupportedLanguage, Record<string, { label: string; help: string }>>> = {
  'de-DE': {
    ...
    'statusBarQuotaFormat': { label: 'Kontingent: Statusleisten-Format', help: 'Leer behält das eingebaute Layout (5h 6% · wk 1%). Sonst: {5h.pct}, {wk.pct} (oder {7d.pct}) und {model:Fable.pct}, jeweils auch mit .reset und .label — z. B. "{5h.pct} | {7d.pct} · {model:Fable.pct}". Ein Fenster, das dein Tarif nicht meldet, bleibt leer.' },
  ...
  'zh-TW': {
    ...
    'statusBarQuotaFormat': { label: '配額：狀態列格式', help: '留空維持內建版面（5h 6% · wk 1%）。否則：{5h.pct}、{wk.pct}（或 {7d.pct}）與 {model:Fable.pct}，每個也可加 .reset 與 .label — 例如「{5h.pct} | {7d.pct} · {model:Fable.pct}」。方案未回報的視窗會顯示為空。' },
  ...
  'zh-CN': {
    ...
    'statusBarQuotaFormat': { label: '配额：状态栏格式', help: '留空保持内置布局（5h 6% · wk 1%）。否则：{5h.pct}、{wk.pct}（或 {7d.pct}）和 {model:Fable.pct}，每个也可加 .reset 与 .label — 例如“{5h.pct} | {7d.pct} · {model:Fable.pct}”。方案未报告的窗口会显示为空。' },
  ...
  'ja': {
    ...
    'statusBarQuotaFormat': { label: 'クォータ：ステータスバーの書式', help: '空なら組み込みのレイアウト（5h 6% · wk 1%）。指定する場合は {5h.pct}、{wk.pct}（または {7d.pct}）、{model:Fable.pct}。いずれも .reset と .label を取れます — 例「{5h.pct} | {7d.pct} · {model:Fable.pct}」。プランが報告しないウィンドウは空になります。' },
  ...
  'ko': {
    ...
    'statusBarQuotaFormat': { label: '할당량: 상태 표시줄 형식', help: '비워 두면 기본 레이아웃(5h 6% · wk 1%)을 사용합니다. 그 외에는 {5h.pct}, {wk.pct}(또는 {7d.pct}), {model:Fable.pct}. 각각 .reset과 .label도 쓸 수 있습니다 — 예: "{5h.pct} | {7d.pct} · {model:Fable.pct}". 요금제가 보고하지 않는 창은 비어 있게 표시됩니다.' },
  ...
  'pt-BR': {
    ...
    'statusBarQuotaFormat': { label: 'Cota: formato da barra de status', help: 'Vazio mantém o layout embutido (5h 6% · wk 1%). Caso contrário: {5h.pct}, {wk.pct} (ou {7d.pct}) e {model:Fable.pct}, cada uma aceitando também .reset e .label — ex.: "{5h.pct} | {7d.pct} · {model:Fable.pct}". Uma janela que seu plano não reporta fica vazia.' },
  ...
  'id': {
    ...
    'statusBarQuotaFormat': { label: 'Kuota: format bilah status', help: 'Kosong mempertahankan tata letak bawaan (5h 6% · wk 1%). Jika tidak: {5h.pct}, {wk.pct} (atau {7d.pct}) dan {model:Fable.pct}, masing-masing juga menerima .reset dan .label — mis. "{5h.pct} | {7d.pct} · {model:Fable.pct}". Jendela yang tidak dilaporkan paket Anda akan kosong.' },
