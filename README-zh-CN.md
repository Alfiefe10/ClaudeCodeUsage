...
- **配额状态栏格式**：`statusBarQuotaFormat`（⚙ 设置）允许你自己指定要显示的配额窗口，适用于内置的 `5h 6% · wk 1%` 不符合预期时：`{5h.pct}`、`{wk.pct}`（或 `{7d.pct}`）与 `{model:Fable.pct}`，每个也可加 `.reset` 与 `.label`。例如 `{5h.pct} | {7d.pct} · {model:Fable.pct}` 会渲染为 `6% | 1% · 12%`。留空（默认）保持内置布局；设置模板后会覆盖上述两个选项，因为模板已经指明了要显示哪些窗口。账户未报告的窗口会连同其分隔符一起消失。
