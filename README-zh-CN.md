...
- **配额状态栏格式**：⚙ 设置中的下拉栏默认采用内置布局，也可选择仅 5 小时、仅每周；选“自定义”时才显示 `statusBarQuotaFormat` 模板输入框。可写 `{5h.pct}`、`{wk.pct}`（或 `{7d.pct}`）与 `{model:Fable.pct}`，每个也可加 `.reset` 与 `.label`。`.reset` 默认跟随 `resetCountdownFormat`，也可自带样式：`{5h.reset:units}`（`4h 48m`）、`:decimal`（`4.8h`）、`:clock`（`18:20`）或 `:at`（时刻，如 `Thu 16:59`）。例如 `{5h.pct} | {7d.pct} · {model:Fable.pct}` 会渲染为 `6% | 1% · 12%`。留空保持内置布局；模板会覆盖上述两个选项。账户未报告的窗口会连同其分隔符一起消失。
